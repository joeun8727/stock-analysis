...
주식 **단타(스캘핑) 백테스팅** 시스템입니다. 코딩 없이 **규칙 조합**(지표 + 조건)으로 매매 로직을 만들고, 분봉 엑셀 데이터(3분봉·1분봉)로 백테스트해서 지정한 투자금액 기준의 성공/실패율, 원화 수익금, 실패 분석, 자동 진단을 받습니다.
...
분봉 엑셀(`.xlsx`)을 올립니다. 종목별로 시장 구분(선물 / ETF / 일반)을 고르면 알맞은 `data/` 하위 폴더에 저장되고, 파싱된 봉이 DB(`price_bar`)에 들어갑니다. 봉 길이(3분봉/1분봉)는 파일의 시각에서 자동으로 읽어 데이터셋에 기록하므로, 같은 종목을 봉 길이만 다르게 따로 올려 비교할 수 있습니다(종목명은 겹치면 안 됩니다). 14MB / 16만 행짜리도 스트리밍 파서로 처리하지만 시간은 좀 걸립니다.
...
검증한 로직을 한국투자증권 계좌로 그대로 돌립니다. **저장한 전략 하나가 백테스트·모의투자·실전에서 똑같이 동작합니다** — 매매 규칙은 전략 화면에만 있고 실투자 설정에는 계좌·투자금·한도만 있습니다.

순서를 코드로 강제합니다: **백테스트를 한 적 없는 전략은 켜지지 않고**, 검증 후 규칙을 고치면 다시 잠깁니다.

1. [한국투자증권 오픈API](https://apiportal.koreainvestment.com)에서 **모의투자** 앱키를 발급받아 `.env`에 넣습니다 (`KIS_MODE=DRY_RUN`으로 시작).
2. `데이터 업로드` 화면에서 레버리지·인버스 데이터셋에 **종목코드**를 입력합니다 (예: `122630`). 표시명으로는 주문할 수 없습니다.
3. `실투자` 화면에서 전략·ETF 그룹·선물 종목코드(최근월물)·투자금·한도를 정합니다.
4. **연결 점검**을 누릅니다 — 주문은 나가지 않고 인증·잔고·시세만 확인합니다.
5. `오늘 자동매매 켜기`. 스위치는 **그날 하루만** 유효하고 자정에 자동으로 풀립니다.

모드는 `.env`의 `KIS_MODE`로만 바뀝니다(재기동 필요). **`DRY_RUN` → `PAPER` → `REAL` 순서로 올라가세요.**

| 모드 | 하는 일 |
|---|---|
| `DRY_RUN` | 실제 시세로 판단하고 기록만 — 주문을 보내지 않음. 며칠 관찰용 |
| `PAPER` | 모의투자 서버에 진짜 주문 |
| `REAL` | 실계좌에 진짜 주문 |

> ⚠️ **실계좌 전에 반드시 확인할 것.** 08:45~09:00은 파생상품 장 개시 전 구간이라, KIS 선물 API가 이 시간에 갱신된 값을 주는지 **실제 장전 시간에 `연결 점검`으로 확인**해야 합니다. 이 값이 곧 매수 방향이라 여기가 틀리면 전부 틀립니다. 주문 `tr_id`도 KIS가 개편한 이력이 있으니 공식 문서와 대조하세요(필요하면 코드 수정 없이 설정으로 덮어쓸 수 있습니다).
...
실투자용(전부 선택 — 비워두면 실투자 기능만 동작하지 않고 백테스트에는 영향이 없습니다):

| 변수 | 설명 |
|---|---|
| `KIS_MODE` | `DRY_RUN`(기본) / `PAPER` / `REAL`. **API로는 못 바꿉니다** — 실계좌 전환에 재기동이 필요하도록 일부러 환경변수로만 뒀습니다 |
| `KIS_APP_KEY` / `KIS_APP_SECRET` | 한국투자증권 오픈API 앱키 |
